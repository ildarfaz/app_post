...
export const postsApi = createApi({
    ...
    endpoints: (builder) => ({
        getPosts: builder.query({
            query: () => `/posts`,
            transformResponse: (response: IPost[], error, { query, page, sortColumn, direction }: IArguments) => {

                const data = [...response].filter(({
                    id,
                    title,
                    body }) => id.toString().includes(query) || title.includes(query) || body.includes(query)).sort(
                        (a, b) => {
                            switch (sortColumn) {
                                case "id":
